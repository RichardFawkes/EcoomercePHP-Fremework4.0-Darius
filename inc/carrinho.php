...
class Carrinho extends conexao{
	...
	public function getProdutos(){
		...
		$sql = 'SELECT p.titulo titulo, p.img , SPLIT_STRING(p.descricao,"<br><br>",1) descricao, c.qtde , c.id idCarrinho , p.id idProduto , p.url , IF(cor.cor IS NULL , "" , CONCAT("Tampa: " , cor.cor)) cor, estoque, quantidade,p.mostra_cor_da_tampa
		, (SELECT valorUnitario FROM PrecosQuantidades WHERE idProduto = c.idProduto AND qtde >= c.qtde ORDER BY valorUnitario DESC LIMIT 1) valorUnitario,c.idProjeto,p.rangeqtde,p.prazo_producao
				FROM Carrinho c
		...
		public function insereProduto($idProduto , $qtde , $idCorTampa=1){
			...
			if($r['qtde']==""){
				$sql = 'INSERT INTO Carrinho (idUser , idProduto , qtde , idCorTampa , dataHora)
				VALUES ('.$_SESSION['idUser'].' , '.$idProduto.' , '.$qtde.' , '.$idCorTampa.' , NOW());';
			}else{
				$qtde = $r['qtde']+$qtde;
				$sql = 'UPDATE Carrinho
				SET qtde = '.$qtde.'
