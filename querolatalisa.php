...
<?php
  ...
  $
  $sqlt = 'SELECT  p.id idProduto ,cp.idTema,cp.idCategoria,p.largura_rotulo,p.altura_rotulo FROM Categorias_X_Produtos cp
...
$idProduto = $ri['idProduto'];
  ...
  $customHeader = 1;
    ...
    $sql4= 'SELECT * From PrecosQuantidades WHERE idProduto='.$ri['idProduto'].'
...
<?php
  ...
  $sql = 'SELECT p.id , p.url, p.titulo , p.img , p.img2 , p.rangeqtde, p.max, p.img3 , p.img4 , p.descricao, p.preco , p.preco_antigo , p.sku, p.mostra_3d , p.mostra_cor_da_tampa, p.quantidade, p.estoque,
...
<div class="container main-container headerOffset">
    ...
    <div class="row transitionfx">
        ...
        <div class="col-lg-6 col-md-6 col-sm-5">
            <h1 class="product-title"> <?php echo $r['titulo']; ?></h1>
...
?>
  <br>
                                      ...
                                      <div class="col-lg-6 col-md-6 col-sm-6 qtd">
  <br>
  <br>
                                      <table class="table table-striped qtd" id="tabela" style="

                              color: #555;
                              margin-bottom: 0;  font-family: Montserrat, sans-serif; text-align: center;  font-size: 11px; font-weight: bold; border: 2px solid #ccc; background-color:white;"">
  <thead>
    <tr>
  ...
  ?>
          ...
          <?php if ($r['mostra_3d']==1) { ?>
            ...
            <?php $sqluser = 'SELECT COUNT(*)total,nome FROM Users WHERE id='.$_SESSION['idUser'].' AND nome IS NOT NULL';
                ...
                if ($rp['total'] == 1) {
                    echo '          <div class="col-lg-6 col-md-6 col-sm-6 col-xs-12"><a class="btn btn-success large linkprintnow"  href='.$editor.''.$r['id'].'&tpsc=1&cru='.$site.'api%2Fcriar-order.php%3Fepi%3D'.$r['id'].'%26url%3D'.$r['url'].'><i class="fa fa-pencil-square"></i> DESIGN ONLINE</a></div>
                    ';
                } else {
...
<?php
    ...
    $sql = 'SELECT id , titulo , img , descricao, preco_antigo ,url FROM Produtos 
    
    WHERE url != "'.$_GET['urlProduto'].'" AND ativo = 1 AND especifico = 0;;';
    $q = mysqli_query($link, $sql);
...
<iframe src="<?php echo $site; ?>3D/uploads/index.php?urlLink=<?php echo $_GET['i'];?>"></iframe>
                    </div>
                </form>
            </div>
        </div>
