...
<?php
	...
	$sql = 'SELECT img , urlLink FROM Carrossel WHERE ativo = 1;';
	...
	while($r = mysqli_fetch_assoc($q)){
		...
		if(is_null($r['urlLink'])){
			echo '<img class="img-responsive" src="'.$site.'images/carrossel/'.$r['img'].'" alt="img">';
		}else{
			echo '<a href="#"><img class="img-responsive" src="'.$site.'images/carrossel/'.$r['img'].'" alt="img"></a>';
		}
		echo '</div>';
	}//while
?>
...
<div class="container main-container">
    ...
    <div class="morePost row featuredPostContainer style2 globalPaddingTop ">
		...
		<?php
			$sql = 'SELECT id , titulo , img , descricao , preco , preco_antigo , url , lancamento FROM Produtos WHERE ativo = 1 ORDER BY lancamento DESC , id DESC LIMIT 8;';
			$q = mysqli_query($link , $sql);
			while($r = mysqli_fetch_assoc($q)){
				if(!is_null($r['preco_antigo']) && $r['preco_antigo'] > 0){
					$preco_antigo = '<span class="old-price">R$'.$r['preco_antigo'].'</span>';
	...
	if($nr > 0){
		...
		';
				...
				while($r = mysqli_fetch_assoc($q)){
					...
					echo '<li><a '.$linkURL.'><img src="'.$site.'images/clientes/'.$r['img'].'" alt="'.$cliente.'"></a></li>';
				}
