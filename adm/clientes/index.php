...
<div class="wrapper">
	<div class="container">
		...
		<div class="row">
			<div class="col-sm-12">
				<h4 class="page-title">Clientes <a href="cadastrar.php" class="btn btn-primary"><i class="fa fa-plus"></i> Cadastrar</a></h4>
			...
			<div class="row">
				<div class="card-box">
					<table id="tabela" class="table table-striped">
						...
						<tbody>
							<?php
								$sql = 'SELECT id, titulo, urlLink, ativo, img,urlLink link
								FROM Clientes c
								ORDER BY c.id DESC;';
								$res = mysqli_query($link,$sql);
								while($row = mysqli_fetch_array($res)){
									echo "<tr id='cliente_".$row['id']."'>";
									echo "<td><a href='".$site."images/clientes/".$row['img']."' style='cursor:zoom-in;' target='_blank'><img src='".$site."images/clientes/".$row['img']."' class='img-responsive' /></a></td>";
									echo "<td>".$row['titulo']."</td>";
									echo "<td>";
										$activeLink = (is_null($row['urlLink']) || $row['urlLink']=="")? "disabled":"";
										echo "<a class='btn btn-default ".$activeLink."' title='Abrir em nova Aba' data-toggle='tooltip' target='_blank' href='".$row['link']."'><i class='fa fa-external-link'></i></a> ";
		  ...
		  $(document).ready(function(){
	    	...
	    	$('#tabela').DataTable();
				...
				$("#tabela").on('click','.delBtn', function(event) {
			     ...
			     var id = this.id;
			     var clickedID = 'id=' + id;
			 		if(confirm("Certeza em apagar este Cliente?")){
			 	    jQuery.ajax({
