...
if(preg_match("/^.*\.(jpg|jpeg|png|gif)$/i", $_FILES['file']['name']) AND $_FILES['file']['error']==0 ) {
  ...
  if ($image->isValid()) {
    ...
    // Atualiza
    $sql = 'UPDATE Categorias SET categoria="'.$_POST['titulo'].'", descricao="'.$_POST['descricao'].'",img="'.$namefile.'", urlLink="'.$url.'" WHERE id ='.$_POST['id'].';';
    $q = mysqli_query($link,$sql);

    if($q){
      echo '<script type="text/javascript">
      window.top.location = "'.$rot.'categorias/index.php";
      </script>
      ';
    }else{
      echo '<script type="text/javascript">
  ...
  // Atualiza
  $sql = 'UPDATE Categorias SET categoria="'.$_POST['titulo'].'",descricao="'.$_POST['descricao'].'", urlLink="'.$url.'" WHERE id ='.$_POST['id'].';';
  $q = mysqli_query($link,$sql);
  if($q){
    echo '<script type="text/javascript">
    window.top.location = "'.$rot.'categorias/index.php";
