...
if(preg_match("/^.*\.(jpg|jpeg|png|gif)$/i", $_FILES['file']['name']) AND $_FILES['file']['error']==0 ) {
  ...
  if ($image->isValid()) {
    ...
    // Insere
    $sql = 'INSERT INTO Categorias (categoria, descricao, img, urlLink, idTipo, ativo) VALUES("'.$_POST['titulo'].'", "'.$_POST['descricao'].'", "'.$namefile.'","'.$url.'", 1, 0);';
  ...
  echo '<script type="text/javascript">
  alert("Necessário uma imagem"); history.back();;
  </script>
  ';
}
