...
if(preg_match("/^.*\.(jpg|jpeg|png|gif)$/i", $_FILES['file']['name']) AND $_FILES['file']['error']==0 ) {
  ...
  if ($image->isValid()) {
    ...
    // Atualiza
    $sql = 'UPDATE Carrossel SET titulo="'.$_POST['titulo'].'", img="'.$namefile.'", urlLink="'.$url.'", mode="'.$_POST['mode'].'" WHERE id ='.$_POST['id'].';';
    $q = mysqli_query($link,$sql);

    if($q){
      echo '<script type="text/javascript">
      window.top.location = "'.$rot.'banners/index.php";
      </script>
      ';
    }else{
      echo '<script type="text/javascript">
  ...
  // Atualiza
  $sql = 'UPDATE Carrossel SET titulo="'.$_POST['titulo'].'", urlLink="'.$url.'", mode="'.$_POST['mode'].'" WHERE id ='.$_POST['id'].';';
  $q = mysqli_query($link,$sql);
  if($q){
    echo '<script type="text/javascript">
    window.top.location = "'.$rot.'banners/index.php";
