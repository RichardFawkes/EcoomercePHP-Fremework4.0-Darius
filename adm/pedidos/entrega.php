...
<div class="wrapper">
    ...
    <br class="hidden-md hidden-lg" />
			...
			<div class="row">
				<div class="col-sm-12">
					<h4 class="page-title">Pedidos: Status Entrega</h4>
				</div>
			</div>
            ...
            <th>Status</th>
                 ...
                 c.statusEntrega, c.codigoRastreio
								FROM Compras c
                ...
                WHERE pago=1 AND enviado=1 AND entregue=0 AND statusProducao=2
							...
							while($row = mysqli_fetch_array($res)){
								if($row['entregue']==0){
									...
									$status4 = '<a id="ent_'.$row['id'].'" class="btn btn-warning ent"><i class="fa fa-remove"></i></a>';
								}else{
									// $status4 = "Entregue";
                ...
                echo "<td>".$row['codigoRastreio']."</td>";
								echo "<td>".$row['statusEntrega']."</td>";
								echo "<td>".$status4."</td>";
								echo "<td>";
				 ...
				 jQuery.ajax({
						 ...
						 success: function (json) {
							 $("#ent_"+id).fadeIn(300, function(){
								 $(this).removeClass("btn-warning btn-success").addClass(json.class);
								 $("#ent_"+id +" i").removeClass("fa-check fa-remove").addClass(json.icon);
							 });
						 }
				 });
				});
