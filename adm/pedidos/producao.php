...
// TODO: Verificar se teremos outro status de produção
?>
...
<div class="wrapper">
    ...
    <br class="hidden-md hidden-lg" />
			...
			<div class="row">
				<div class="col-sm-12">
					<h4 class="page-title">Pedidos: Em Produção</h4>
				</div>
			</div>
			...
			<div class="row">
                ...
                WHERE pago=1 AND enviado=0 AND entregue=0 AND statusProducao!=2
							...
							while($row = mysqli_fetch_array($res)){
								...
								echo "<td>".$status1."</td>";
								echo "<td>
								<select name='status' class='form-control spr' id='spr_".$row['id']."'>
									<option value='0' ".$selected0.">Não Produzido</option>
									<option value='1' ".$selected1.">Em Produção</option>
									<option value='2' ".$selected2.">Produzido</option>
								</select>
								</td>";
								echo "<td>";
		...
		<script type="text/javascript">
		  $(document).ready(function(){
            ...
            "targets": 4
				...
				// Produção
				$("#tabela").on('change','.spr', function(event) {
				 event.preventDefault();
				 var id = this.id.substr(4);
				 var status = $(this).val();
				 jQuery.ajax({
						 type: "POST", // HTTP method POST or GET
						 url: "altera-producao.php", //Where to make Ajax calls
						 dataType: "json", // Data type, HTML, json etc.
						 data: {status:status, id:id}, //Form variables
						 success: function (json) {
							 alert(json.msg)
						 }
				 });
