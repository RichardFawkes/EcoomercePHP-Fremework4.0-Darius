...
<div class="wrapper">
	...
	$sqlprodutotitulo = 'SELECT titulo FROM Produtos WHERE id = "'.$_POST['produtoname'].'"';
	$pds = mysqli_query($link,$sqlprodutotitulo);
	$ptitulo =  mysqli_fetch_assoc($pds);

	?>
<h4 class="page-title">Exporta <a class="btn btn-primary" href="exporta-relatorio-alls.php?id=<?php echo $_POST['produtoname']?>&pago=<?php echo $_POST['pago']?>&datainicio=<?php echo $_POST['datainicio']?>&datafinal=<?php echo $_POST['datafinal']?>"><i class="fa fa-download"></i> Exportar em .CSV</a></h4>

	<h4 class="page-title">Relatorio de Pedidos:</h4>
	<form  name="form1" action="resultado.php"method="POST">

<label >Produto *</label>   &nbsp;	<select  id="produtoname" name="produtoname" class="btn btn-success " required> 
	<option value="<?php echo $_POST['produtoname'];?>"><?php echo $ptitulo['titulo'];?></option>

<?php 
while($ap = mysqli_fetch_assoc($pd)){

echo '<option value="'.$ap['id'].'">'.$ap['titulo'].'</option>';
}
?>
</select> &nbsp; &nbsp;


		  <label >Data-Inicial </label>	<input  type="date" id="datainicio" name="datainicio" placeholder="Date"  class="btn btn-success " onfocus="(this.type='date')" required />
...
<br>
   ...
   $idProduto = $_POST['produtoname'];

							$sql = 'SELECT (c.id)idCompra,u.nome,c.idUser,es.estado,pd.titulo,cp.qtde,pq.valorUnitario,c.pago,formata_data(c.dataHora)data,ci.processorName
							...
							WHERE pago = '.$pago.' and DATE( c.dataHora ) between "'.$datainicio.'" AND "'.$datafinal.'" AND pd.id = '.$idProduto.'
