...
<div class="wrapper">
	...
	$sqlcategoria = 'SELECT * FROM Categorias WHERE id = '.$_POST['categoria'].'';
	$pds = mysqli_query($link,$sqlcategoria);

	$ptitulo =  mysqli_fetch_assoc($pds);

	// $datainiciof = formata_data($_POST['datainicio']);
	// $datafinalf = formata_data($_POST['datainicio']);
	?>
<h4 class="page-title">Exporta <a class="btn btn-primary" href="exporta-relatorio-categoria.php?id=<?php echo $_POST['categoria']?>&pago=<?php echo $_POST['pago']?>&datainicio=<?php echo $_POST['datainicio']?>&datafinal=<?php echo $_POST['datafinal']?>"><i class="fa fa-download"></i> Exportar em .CSV</a></h4>

	<h4 class="page-title">Relatorio de Pedidos Categoria </h4> <br>Data Inicio - <?php echo $_POST['datainicio'];?> <br>  Data Final - <?php echo $_POST['datafinal'];?>
	<form  name="form1" action="resultado-categoria.php"method="POST">

<label >Categoria </label>   &nbsp;	<select  id="categoria" name="categoria" class="btn btn-success " required> 
	<option value="<?php echo $_POST['categoria'];?>"><?php echo $ptitulo['categoria'];?></option>
...
?>
		  ...
		  <label >Data-Inicial </label>	<input  type="date" id="datainicio" name="datainicio"  class="btn btn-success "  required />
		  <label >Data-Final </label>	<input type="date" id="" id="datafinal" name="datafinal" class="btn btn-success " required><br>
		  ...
		  <label >PAGO </label>	<select  class="btn btn-success " id="pago" name="pago"  required> 
				...
				<br>
				<br>
<button type="submit" class="btn btn-success" onclick="valida()">GERAR RELATORIO</button>
</form>
<br>
<br>
			<!-- Formulário -->
   ...
   $categoria = $_POST['categoria'];
							...
							$sql = 'SELECT (c.id)idCompra,u.nome,c.idUser,es.estado,pd.titulo,cp.qtde,pq.valorUnitario,c.pago,formata_data(c.dataHora)data,cat.Categoria,cxp.idCategoria,ci.processorName,(cp.qtde * pq.valorUnitario )total
							...
							LEFT JOIN Compras_X_Invoices ci ON ci.idCompra = c.id
							...
							WHERE pago = '.$pago.' and DATE( c.dataHora ) between "'.$datainicio.'" AND "'.$datafinal.'" AND cxp.idCategoria =  '.$categoria.'
						  group by titulo,nome order by idCompra
								';
							...
							while($row = mysqli_fetch_array($res)){
								...
								echo "<td>".formata_real($row['total'])."</td>";
