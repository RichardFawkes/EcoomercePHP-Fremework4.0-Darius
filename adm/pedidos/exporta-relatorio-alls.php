...
// Imprime o conteúdo da nossa tabela no arquivo que será gerado
    ...
    $sql = 'SELECT (c.id)idCompra,u.nome,c.idUser,es.estado,pd.titulo,cp.qtde,pq.valorUnitario,c.pago,formata_data(c.dataHora)data,
    (cp.qtde * pq.valorUnitario )total
    ...
    WHERE pago = '.$_GET['pago'].' and DATE( c.dataHora ) between "'.$_GET['datainicio'].'" and "'.$_GET['datafinal'].'" and pd.id = "'.$_GET['id'].'" group by titulo,nome order by idCompra';
