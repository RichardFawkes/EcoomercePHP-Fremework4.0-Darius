<?php
	...
	$url = 'api/criar-order.php?epi='.$_GET['id'].'&url='.$_GET['url'].'';
	...
	if(is_null($_POST['email']) || $_POST['email'] == '' || is_null($_POST['senha']) || $_POST['senha'] == ''){
        ...
        if($nr == 0){
                echo '<script type="text/javascript">
                        alert("Dados Incorretos! Tente novamente...");
                        window.location = "'.$site.'login-editor?='.$_GET['id'].'";
                </script>';
	...
	if ($r['ativo'] == '1' && $senhaCorreta == 1) {
		...
		mysqli_query($link , $sql);
                ...
                echo '<script>
                window.location.href = "'.$editor.''.$_GET['id'].'&tpsc=1&cru='.$site.''.urlencode($url).'";
                </script>';

        }else{
                echo '<script type="text/javascript">
                        alert("Dados Incorretos! Tente novamente...");
                        window.location = "'.$site.'login-editor?id='.$_GET['id'].'";
                </script>
                ';
                session_destroy();
        }
